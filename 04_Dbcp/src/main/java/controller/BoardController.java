...
@WebServlet("*.do")		// getAllBoards.do,	getBoardByNO.do,  , writeBoard.do   addBoard.do,	modifyBoard.do,		removeBoard.do
public class BoardController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String urlMapping = requestURI.substring(contextPath.length());	/*  /getAllBoards.do		  */
		// 모든 서비스의 공통 타입 선언
		...
		case "/removeBoard.do":
			...
			break;
		case "/writeBoard.do":
			af = new ActionForward("board/write.jsp", false);	// board폴더 아래 write.jsp로 포워드한다 (단순 이동의 경우 포워드한다.)
			break;
		}
		// 서비스 실행
		if(service != null) {
